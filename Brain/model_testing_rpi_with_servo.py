...
try:
    ...
    key_df = pd.read_csv(os.path.join(KEY_DIR,f'voice_data_testing_{today}.csv'))
except:
    ...
    key_df.to_csv(os.path.join(KEY_DIR,f'voice_data_testing_{today}.csv'),index=False)
    key_df = pd.read_csv(os.path.join(KEY_DIR,f'voice_data_testing_{today}.csv')) 
...
model = tf.keras.models.load_model(os.path.join(KEY_DIR,'models/6th_version/voice_button_model_lstm_2020-11-18_8.h5'))
...
while True:
    try:
        ...
        if pred[0] > 0.9 or pred[1]>0.9 or pred[2]>0.9 or pred[3]>0.9:
            ...
            df1.to_csv(os.path.join(KEY_DIR,f'voice_data_testing_{today}.csv'),mode='a',index=False,header=False)
            frames.append(frames0)
            frames.append(frames1)
            frames.append(frames2)
